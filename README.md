...
It's important to be pragmatic about this. Sometimes, an internal representation that differs from the protocol buffer definition is needed, as some things cannot be represented in protocol buffers, and some needed changes make break backwards compatibility of protocol buffers, if used for external APIs. In cases such as these, consider *strongly basing* the internal representation on the protocol buffer definition by sub-classing the code representation generated by protoc for your language or embedding it in your own type.
### Consumer-driven interfaces
...
A more full implementation of this example service can be found in this repository.
## Resources
